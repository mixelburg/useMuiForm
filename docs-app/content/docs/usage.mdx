...
When the stored model value differs from the value the component wants (e.g. you
store a `"YYYY-MM"` string but a month picker works with a dayjs object), pass a
`transform`. `input` maps the stored value → the component on read; `output` maps
the component value → the stored value on change. This replaces the `<Controller>`
boilerplate you'd otherwise write just for conversion.
...
const ymToDayjs = (ym: string) => (ym ? dayjs(ym, 'YYYY-MM') : null)
const dayjsToYm = (d: dayjs.Dayjs | null) => (d ? d.format('YYYY-MM') : '')
...
The returned `value` is typed as whatever `input` returns (here `dayjs.Dayjs | null`),
and `onChange` stores `output(componentValue)`. Works for both raw-value components
(pickers call `onChange(value)`) and DOM-event inputs.
